const Results = ({ score, categoryScores, recommendations }) => {
  ...
  const getGradeInfo = (score) => {
    if (score >= 90) return {
      grade: 'Excellent - Industry Leading',
      color: 'text-green-600',
      bgColor: 'bg-green-100',
      message: 'Your organization has implemented comprehensive cyber resilience measures.'
    };
    if (score >= 75) return {
      grade: 'Good - Well Protected',
      color: 'text-green-500',
      bgColor: 'bg-green-50',
      message: 'Your data protection is strong but has room for improvement.'
    };
    if (score >= 60) return {
      grade: 'Fair - Some Gaps',
      color: 'text-orange-600',
      bgColor: 'bg-orange-50',
      message: 'Several important cyber resilience capabilities are missing.'
    };
    if (score >= 40) return {
      grade: 'Poor - Significant Gaps',
      color: 'text-red-600',
      bgColor: 'bg-red-50',
      message: 'Your organization faces substantial cyber resilience risks.'
    };
    return {
      grade: 'Critical - High Risk',
      color: 'text-red-700',
      bgColor: 'bg-red-100',
      message: 'Immediate action needed to protect against ransomware and data loss.'
    };
  };
      ...
      {recommendations.length > 0 && (
        <div className="mt-8 bg-green-50 p-6 rounded-xl">
          ...
          <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg mb-6">
            <h4 className="font-semibold text-yellow-800 mb-2">📊 Improvement Potential</h4>
          ...
          <div className="mt-8 bg-gradient-to-r from-purple-600 to-indigo-600 text-white p-6 rounded-xl text-center">
            <h3 className="text-2xl font-bold mb-3">Ready to Improve Your Cyber Resilience?</h3>
            <p className="mb-4">Rubrik's Zero Trust Data Security platform addresses all these gaps with a comprehensive solution.</p>
            <button className="bg-white text-purple-600 px-8 py-3 rounded-lg font-bold text-lg hover:shadow-lg transform hover:-translate-y-1 transition-all duration-200">
              Learn How Rubrik Can Help →
            </button>
          </div>
        </div>
      )}
